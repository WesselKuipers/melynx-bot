...
export default class Session {
  constructor() {
    ...
    this.handleExpiredSession = async (client, channel, prefix,
      sessionTimeout, sessionRefreshTimeout, session) => {
      ...
      sentMessage.awaitReactions((r, user) => (r.emoji.name === '♻' && user.id !== client.user.id), { max: 1, time: sessionRefreshTimeout })
        .then((collected) => {
          ...
          SessionDb.create({
            ...
            .then((dbSes) => {
              Object.assign(dbSes, {
                timer: setTimeout(
                  () => this.handleExpiredSession(client, channel, prefix,
                    sessionTimeout, sessionRefreshTimeout, dbSes),
                  sessionTimeout,
                ),
              });
              sessions.push(dbSes);
   ...
   */
    this.updateSessionMessage = async (client, conf) => {
       ...
       */
        const channel = client.channels.find(c => c.id === conf.sessionChannel.replace(/<|#|>/g, ''));
   ...
   */
    this.run = async (client, message, conf, params) => {
      if (conf.sessionChannel && !sessionChannelTimers[message.guild.id]) {
        const timer = setInterval(async () => this.updateSessionMessage(client, conf), 5 * 60 * 1000);
      ...
      const {
        ...
        sessionTimeout,
        sessionRefreshTimeout,
      } = { ...client.defaultSettings, ...conf, guildId: message.guild.id };
      ...
      dbSes.timer = setTimeout(
        () => this.handleExpiredSession(client, message.channel, prefix,
          sessionTimeout, sessionRefreshTimeout, dbSes),
        sessionTimeout,
