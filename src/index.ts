...
export class MelynxBot {
  ...
  async onCommandInteraction(client: MelynxClient, interaction: Interaction) {
    ...
    if (interaction.isAutocomplete()) {
      ...
      if (command?.autocompleteExecute) {
        try {
          await command.autocompleteExecute(interaction, this.client);
        } catch (error) {
          this.client.error(error as Error);
        }
      }
  ...
  run() {
    ...
    this.client.on(Events.Error, (error) => this.client.error(error));
    this.client.on(Events.Warn, (warning) => this.client.warn(warning));
